...
import { MANAGED_HOOK_SERVICE_CENSUS } from './managed-hook-service-census'
...
`
  ...
  it('registers every managed agent that implements installRemote in the remote installer (issue #7253)', () => {
    const servicesByAgent = MANAGED_HOOK_SERVICE_CENSUS

    // Guard against a service silently missing from the census as new agents land.
